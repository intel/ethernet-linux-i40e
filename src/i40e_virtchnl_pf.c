...
#define I40E_MAX_TX_QLEN 0x1FE0
				    ...
				    struct virtchnl_txq_info *info)
{
	...
	if (info->ring_len & 7 || info->ring_len > I40E_MAX_TX_QLEN) {
		ret = -EINVAL;
		goto error_context;
	}
...
#define I40E_MAX_RX_QLEN 0x1FE0
				    ...
				    struct virtchnl_rxq_info *info)
{
	...
	if (info->ring_len & 31 || info->ring_len > I40E_MAX_RX_QLEN) {
		ret = -EINVAL;
		goto error_param;
	}
					  ...
					  const u8 *macaddr)
{
	...
	mac_elem = (struct i40e_vm_mac *)kzalloc(sizeof(*mac_elem), GFP_ATOMIC);
...
static int i40e_vc_get_vf_resources_msg(struct i40e_vf *vf, u8 *msg)
{
	...
	vfres = (struct virtchnl_vf_resource *)kzalloc(len, GFP_KERNEL);
	if (!vfres) {
					   ...
					   s16 **vlan_list)
{
	...
	*vlan_list = (s16 *)kcalloc(*num_vlans, sizeof(**vlan_list),
				    GFP_ATOMIC);
	if (!(*vlan_list))
		goto err;
...
static int i40e_vc_config_queues_msg(struct i40e_vf *vf, u8 *msg)
{
	...
	for (i = 0; i < qci->num_queue_pairs; i++) {
		...
		if (vf->adq_enabled) {
			if (idx >= ARRAY_SIZE(vf->ch) || idx >= vf->num_tc) {
				 ...
				 bool enable)
{
	...
	if (test_bit(__I40E_PORT_SUSPENDED, pf->state) && enable)
		return ret;
...
{
	i40e_status v_ret = I40E_SUCCESS;
	struct virtchnl_vlan_filter_list *vfl =
...
static int i40e_vc_get_offload_vlan_v2(struct i40e_vf *vf)
{
	...
	caps = (struct virtchnl_vlan_caps *)kzalloc(sizeof(*caps), GFP_KERNEL);
	if (!caps) {
...
static int i40e_vc_add_vlan_v2_msg(struct i40e_vf *vf, u8 *msg)
{
	...
	vfl = (struct virtchnl_vlan_filter_list *)kzalloc
		(sizeof(*vfl) + vfl_v2->num_elements * sizeof(u16),
		 GFP_KERNEL);
	if (!vfl) {
		v_ret = VIRTCHNL_STATUS_ERR_NO_MEMORY;
		goto out;
